...
playoff_data = {
    ...
    "Jan Hanuš": {"matches": 4, "wins": 2, "winRate": 50, "singl": {"matches": 1, "wins": 0, "winRate": 0}, "dvojice": {"matches": 1, "wins": 1, "winRate": 100}, "trojice": {"matches": 2, "wins": 1, "winRate": 50}},
    ...
    # Solidarita
    "Landrichter Tomáš": {"matches": 4, "wins": 2, "winRate": 50, "singl": {"matches": 0, "wins": 0, "winRate": 0}, "dvojice": {"matches": 1, "wins": 1, "winRate": 100}, "trojice": {"matches": 3, "wins": 1, "winRate": 33}},
    "Kozár Martin": {"matches": 7, "wins": 3, "winRate": 43, "singl": {"matches": 0, "wins": 0, "winRate": 0}, "dvojice": {"matches": 3, "wins": 2, "winRate": 67}, "trojice": {"matches": 4, "wins": 1, "winRate": 25}},
    "Šafránek Jan": {"matches": 7, "wins": 3, "winRate": 43, "singl": {"matches": 1, "wins": 0, "winRate": 0}, "dvojice": {"matches": 3, "wins": 2, "winRate": 67}, "trojice": {"matches": 3, "wins": 1, "winRate": 33}},
    "Dvořák Luděk": {"matches": 8, "wins": 3, "winRate": 38, "singl": {"matches": 1, "wins": 1, "winRate": 100}, "dvojice": {"matches": 3, "wins": 1, "winRate": 33}, "trojice": {"matches": 4, "wins": 1, "winRate": 25}},
    "Nováček Ivo": {"matches": 6, "wins": 2, "winRate": 33, "singl": {"matches": 0, "wins": 0, "winRate": 0}, "dvojice": {"matches": 2, "wins": 1, "winRate": 50}, "trojice": {"matches": 4, "wins": 1, "winRate": 25}},
    "Prachár Matej": {"matches": 6, "wins": 2, "winRate": 33, "singl": {"matches": 0, "wins": 0, "winRate": 0}, "dvojice": {"matches": 3, "wins": 1, "winRate": 33}, "trojice": {"matches": 3, "wins": 1, "winRate": 33}},
    "Mikula Daniel": {"matches": 1, "wins": 0, "winRate": 0, "singl": {"matches": 0, "wins": 0, "winRate": 0}, "dvojice": {"matches": 0, "wins": 0, "winRate": 0}, "trojice": {"matches": 1, "wins": 0, "winRate": 0}},
}
...
# Find and process each team
teams_pattern = r'(\{[^{]*"id": "[A-Z]{4}"[\s\S]*?"players": \[[\s\S]*?\][\s\S]*?\}),?\s*(?=\{[^{]*"id": "[A-Z]{4}"|$)'
...
def add_playoff_to_player(player_text, player_name):
    ...
    # Find where to insert (after seasonStats, before availableSkills)
    insert_pattern = r'(\s*\],\s*"availableSkills":)'
    ...
    updated = re.sub(insert_pattern, playoff_json + r'\1', player_text)

    if updated != player_text:
        print(f"  Pridany playoff statistiky pro {player_name}")
        return updated
    else:
        print(f"  Nepodarilo se pridat playoff data pro {player_name}")
        return player_text
...
updated_content = re.sub(player_pattern, process_player, updated_content)
