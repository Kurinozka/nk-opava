  ...
  team.players.forEach(player => {
    ...
    if (issues.length > 0) {
      const formattedIssues = issues.map(issue => ({
        ...
        chybne: {
          singl: issue.singl,
          dvojice: issue.dvojice,
          trojice: issue.trojice
        },
        spravne: {
          singl: 0,
          dvojice: issue.dvojice,
...
const result = {
  summary: {
    totalProblematicPlayers: problematicPlayers.length,
    totalProblematicRecords: problematicPlayers.reduce((sum, p) => sum + p.issues.length, 0)
  },
  explanation: {
    problem: 'V nohejbale NELZE hrat soucasne singl (jednotlivci) A dvojice/trojice (tymove discipliny)',
    rules: [
      'POUZE singl (pak dvojice = 0, trojice = 0)',
      'POUZE dvojice/trojice (pak singl = 0)'
    ],
    fix: [
      'Nastavit disciplines.singl.matches = 0',
      'Hodnotu z disciplines.singl.matches presunout do matches (celkovy pocet zapasu)',
      'Hodnoty disciplines.dvojice a disciplines.trojice zachovat'
    ]
  },
  problematicPlayers: problematicPlayers
};
