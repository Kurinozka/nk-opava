...
// Pro extraligu - načtu celý soubor
const extraligaContent = fs.readFileSync(extraligaTeamsPath, 'utf8');

// Vytvořím dočasný modul pro načtení dat
const tempPath = 'C:\\Users\\ASUS\\Desktop\\temp_extraliga.js';
fs.writeFileSync(tempPath, extraligaContent);
delete require.cache[require.resolve(tempPath)];
const { extraligaTeams } = require(tempPath);
...
console.log(`Zkontrolováno hráčů z NK Opava: ${nkOpavaCount}`);
console.log(`Zkontrolováno hráčů z extraligy: ${extraligaCount}`);
console.log(`Celkem zkontrolováno: ${nkOpavaCount + extraligaCount}\n`);
...
if (totalErrors > 0) {
  ...
  if (errors.disciplineSum.length > 0) {
    ...
    errors.disciplineSum.forEach(err => {
      ...
      } else if (err.actual === 0) {
        console.log(`  ✓ DOPORUČENÍ: Všechny disciplíny mají 0 zápasů, ale matches je ${err.expected}`);
        console.log(`    Buď přidat data disciplín, nebo změnit matches na 0`);
      } else {
  ...
  if (errors.dataValidity.length > 0) {
    console.log('\n\nOPRAVY SPRÁVNOSTI DAT:');
    errors.dataValidity.forEach(err => {
      console.log(`\n${err.player} (${err.team}) - ${err.season}:`);
      console.log(`  ✓ DOPORUČENÍ: V disciplíně "${err.discipline}" jsou wins větší než matches`);
